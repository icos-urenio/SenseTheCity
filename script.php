...
class com_sensethecityInstallerScript
{
	/*
	...
	function preflight( $type, $parent ) {
		...
		if( version_compare( $jversion->getShortVersion(), $this->minimum_joomla_release, 'lt' ) ) {
			Jerror::raiseWarning(null, 'Cannot install com_sensethecity in a Joomla release prior to '.$this->minimum_joomla_release);
			return false;
		}
		...
		//echo '<p>' . JText::_('COM_SENSETHECITY_PREFLIGHT_' . $type . ' ' . $rel) . '</p>';
	...
	function install( $parent ) {
		//echo '<p>' . JText::_('COM_SENSETHECITY_INSTALL to ' . $this->release) . '</p>';
		
		// You can have the backend jump directly to the newly installed component configuration page
		// $parent->getParent()->setRedirectURL('index.php?option=com_sensethecity');
	}
	...
	function update( $parent ) {
		//echo '<p>' . JText::_('COM_SENSETHECITY_UPDATE_ to ' . $this->release) . '</p>';
		$params['version'] = 'SenseTheCity version ' . $this->release;
	...
	function postflight( $type, $parent ) {
		...
		//echo '<p>' . JText::_('COM_SENSETHECITY_POSTFLIGHT ' . $type . ' to ' . $this->release) . '</p>';
	}
	...
	function uninstall( $parent ) {
		//echo '<p>' . JText::_('COM_SENSETHECITY_UNINSTALL ' . $this->release) . '</p>';
	}
	...
	function getParam( $name ) {
		$db = JFactory::getDbo();
		$db->setQuery('SELECT manifest_cache FROM #__extensions WHERE name = "com_sensethecity"');
	...
	function setParams($param_array) {
		if ( count($param_array) > 0 ) {
			...
			$db = JFactory::getDbo();
			$db->setQuery('SELECT params FROM #__extensions WHERE name = "com_sensethecity"');
